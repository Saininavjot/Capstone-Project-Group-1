...
int gpio_control_init(int pin, pin_direction state)
{
    ...
    {
        printf("pin no:%d put of scope", pin);
        return -1;
    }
	...
	{
		printf("success exporting pin:%d\r\n", pin);
	}
    ...
    if (state == pin_direction_input)
    {
        ...
        {
            printf("Error writing to /sys/class/gpio/gpio%d/directionr\r\n", pin);
            close(fd);
            goto error;
        }
        {
            printf("success open dir file for pin:%d\r\n", pin);
        }
    }
    else if (state == pin_direction_output)
    {
        if (write(fd, "out", 3) != 3) {
            printf("Error writing to /sys/class/gpio/gpio%d/direction\r\n", pin);
            close(fd);
            goto error;
        }
        {
